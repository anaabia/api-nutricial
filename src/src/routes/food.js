...
const { isValidId, isValidArray } = require('../../helpers/id.helper');
...
module.exports = app => {
  ...
  app.get(`${API_ROUTE}/food/list/:listCodes`, (req, res) => {
    const { listCodes } = req.params;
    // if (!isValidArray(listCodes)) {
    //   res.status(400).json({
    //     message: 'Invalid Food ID',
    //   });
    // }

    res.json(FoodController.getListFoodById(listCodes.split(",").map(vlr => parseInt(vlr))));
  });
  ...
  app.get(`${API_ROUTE}/category/:categoryId/food`, (req, res) => {
    ...
    /**
   * @api {get} /food/category/list/:categoryId/food Request all food from a specific category
   * @apiName GetFoodListByCategory
   * @apiGroup Food
   * @apiExample {curl} Example usage:
   *     curl -i http://localhost:4000/api/v1/category/1/food
   ...
   *
   */
  app.get(`${API_ROUTE}/food/category/list/:listCategoryId`, (req, res) => {
    const { listCategoryId } = req.params;
    ...
    //   });
    // }
    console.log(listCategoryId.split(",").map(vlr => parseInt(vlr)));
    res.json(FoodController.getListFoodByCategoryId(listCategoryId.split(",").map(vlr => parseInt(vlr))));
  });
};
